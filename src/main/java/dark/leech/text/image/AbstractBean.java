...
public abstract class AbstractBean {
     ...
     * PropertyChangeSupport cannot be extended directly because it requires a bean in the
     * constructor, and the "this" argument is not valid until after super construction. Hence,
     * delegation instead of extension
     */
     ...
     * the same event source, it will be notified one less time after being removed. If <code>
     * listener</code> is null, or was never added, no exception is thrown and no action is taken.
     *
     ...
     * to see if it's a <code>PropertyChangeListenerProxy</code>, perform the cast, and examine the
     * parameter.
     *
     * <pre>
     ...
     *     If no such listeners have been added, or if <code>propertyName</code> is null, an empty
     *     array is returned.
     */
     ...
     *
     * <p>This is merely a convenience wrapper around the more general firePropertyChange method
     * that takes {@code PropertyChangeEvent} value.
     *
     ...
     * The same listener object may be added more than once, and will be called as many times as it
     * is added. If <code>listener</code> is null, no exception is thrown and no action is taken.
     *
     ...
     * <p>
     *
     * @return ListExecute of VetoableChangeListeners and VetoableChangeListenerProxys if named
     *     property change listeners were added.
     */
    ...
    /**
     * Add a VetoableChangeListener for a specific property. The listener will be invoked only when
     * a call on fireVetoableChange names that specific property. The same listener object may be
     * added more than once. For each property, the listener will be invoked the number of times it
     * was added for that property. If <code>propertyName</code> or <code>listener</code> is null,
     * no exception is thrown and no action is taken.
     *
     ...
     *
     * @param propertyName The name of the property being listened to
     * @return all the <code>VetoableChangeListeners</code> associated with the named property. If
     *     no such listeners have been added, or if <code>propertyName</code> is null, an empty
     *     array is returned.
     */
    ...
    /**
     * Report a vetoable property update to any registered listeners. If anyone vetos the change,
     * then fire a new event reverting everyone to the old value and then rethrow the
