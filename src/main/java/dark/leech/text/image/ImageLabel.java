...
public class ImageLabel extends JLabel {
    ...
    public void load() {
        new ImageLoader().url(urlImage).path(pathImage).input(imageStream).loadTo(this).execute();
    }
}
...
class ImageLoader extends SwingWorker<InputStream, Void> {
    ...
    protected InputStream doInBackground() throws Exception {
        if (urlImage != null)
            if (pathImage != null) {
                ...
                // Use WebP conversion for cover images (ending with cover.jpg)
        ...
        if (pathImage != null) {
            if (new File(pathImage).exists())
                if (new File(pathImage).isFile()) imageStream = new FileInputStream(pathImage);
        }
        if (imageStream == null)
    ...
    private void setImage(InputStream in) {
        try {
            ...
            ImageIcon icon =
                    new ImageIcon(image.getScaledInstance(dx, dy, BufferedImage.SCALE_SMOOTH));
            label.setIcon(icon);
        } catch (Exception ex) {
            Log.add(ex);
        }
