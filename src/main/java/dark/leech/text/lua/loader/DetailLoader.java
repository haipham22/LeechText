...
public class DetailLoader {
    ...
    public BookEntity load(String url) {
        if (!TextUtils.isEmpty(plugin.getDetailGetter())) {
            ...
            if (!value.get("cover").isnil()) entity.setCover(value.get("cover").tojstring());
            if (!value.get("author").isnil()) entity.setAuthor(value.get("author").tojstring());
